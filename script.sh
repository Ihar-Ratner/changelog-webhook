...
LATEST_RELEASE=$(curl -s https://api.github.com/repos/hashicorp/terraform-provider-azurerm/releases/latest | jq -r ".tag_name")
...
if [ "$LATEST_RELEASE" != "$LAST_RELEASE" ]; then
    ...
    response=$(curl https://api.github.com/repos/hashicorp/terraform-provider-azurerm/releases/latest)
